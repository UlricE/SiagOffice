...
/* someone didn't get -DSIAG automatically; this is a workaround */
#ifndef SIAG
#define SIAG
#endif
...
{setvar(cintern("*slib-version*"),
	cintern("$Id: slib.c,v 1.1.1.1 2003/01/15 23:55:12 ulric Exp $"),
	NIL);}

char *siod_version(void)
{return("3.4 18-FEB-97");}
...
/* Siag uses these to interface with Siod, so they must not be static */
#ifndef SIAG
static
#endif	/* SIAG */
 char *repl_c_string_arg = NULL;
#ifndef SIAG
static
#endif	/* SIAG */
 long repl_c_string_flag = 0;

#ifndef SIAG
static
#endif	/* SIAG */
 LISP repl_c_string_read(void)
{LISP s;
...
#if !defined(__osf__) & !defined(VMS) & !defined(SUN5)
double myrealtime(void)
{time_t x;
 time(&x);
 return((double) x);}
...
{long flag;
 flag = no_interrupt(1);
#ifdef SIAG
 if (f == stdout) printf("%s", st);	/* ends up in statusprint */
 else fprintf(f, "%s", st);
#else
 fprintf(f,"%s",st);
#endif	/* SIAG */
 ...
 for(l=sl;NNULLP(l);l=CDR(l))
   if (strcmp(name,PNAME(CAR(l))) == 0)
     {no_interrupt(flag);
      return(CAR(l));}
 if (copyp == 1)
   {cname = (unsigned char *) must_malloc(strlen(name)+1);
    strcpy((char *)cname,name);}
 else
   cname = (unsigned char *)name;
...
int siod_verbose_check(int level)
