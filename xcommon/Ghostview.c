    ...
    Widget w;
{
    ...
    if (gvw->ghostview.input_buffer) MwFree(gvw->ghostview.input_buffer);
#ifdef VMS
    if (gvw->ghostview.output_buffer) MwFree(gvw->ghostview.output_buffer);
#endif /* VMS */
...
#define vfork fork
	...
	sprintf(b, "GHOSTVIEW=%s", buf);
	putenv(b);
	sprintf(c, "DISPLAY=%s", XDisplayString(XtDisplay(w)));
	putenv(c);
...
/* GhostviewSetAntialiasing:
 */
void
GhostviewSetAntialiasing(w)
    Widget w;
{
    antialiasing = True;
}

/* GhostviewUnsetAntialiasing:
 */
void
GhostviewUnsetAntialiasing(w)
    Widget w;
{
    antialiasing = False;
}
    ...
    Bool close;
{
    ...
    ps_new = (struct record_list *) MwMalloc(sizeof (struct record_list));
    ps_new->fp = fp;
    ps_new->begin = begin;
    ps_new->len = len;
	...
	gvw->ghostview.input_buffer = MwMalloc(GV_BUFSIZ);
    }
    ...
    Widget w;
{
    ...
    */
    ...
    if (antialiasing)
        strcat(cmd, " \"-sDEVICE=x11alpha\"");
    else
        strcat(cmd, " \"-sDEVICE=x11\"");
    strcat(cmd, " \"-dNOPAUSE\" ");
    ...
    */
	...
	gvw->ghostview.output_buffer = MwMalloc(GV_BUFSIZ);
    }
    GLInsert(gvw);
    /*
